...
def validate_count(table_name):
      ...
      if stg == src:
            return df.dropDuplicates(['VenueId'])
      else:
            raise_error('Table {} records count is not matching source'.format(table_name))

vnu_df = validate_count('evnt_pltfrm_venue')
vnu_tb = vnu_df.createOrReplaceTempView("temp_venue")
...
vnu_tf.write.format("jdbc").option("url","jdbc:postgresql://172.21.0.7:5432/airflow")\
    .option("driver","org.postgresql.Driver").option("dbtable",'"Event_Dwh"."Venue"')\
    .option("user","airflow")\
...
for row in vnu_tf_update.collect():
      ...
      .option("driver","org.postgresql.Driver")\
      .option("query","""update "Event_Dwh"."Venue" 
                              set venue_name='{}' ,UpdateDt_id='{}'
                              where src_Venue_id = {}
                  """.format(row['venue_name'], row['UpdateDt_id'], row['src_Venue_id']))\
      .option("user","airflow")\
      .option("password","airflow").save()