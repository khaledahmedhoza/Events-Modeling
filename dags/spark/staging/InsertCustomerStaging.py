...
def validate_fields_type(source_df):
  tb_schema = StructType(
      List(
        ...
        StructField("Pmail",StringType, True),
...
cust_df = validate_count('Customer')

#check existence of columns
quinn.validate_presence_of_columns(cust_df, ["CustId", "FirstName", "LastName", "Pmail", "CreateDate", "UpdateDate","PhoneNo"])
#Check Columns Types
validate_fields_type(cust_df)

#valid df
#df = cust_df.withColumn('BatchId',BatchId)
df = cust_df
#insert df to hive staging db
df.write.mode("append").saveAsTable("staging.evnt_pltfrm_customer")
