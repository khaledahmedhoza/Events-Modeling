...
def validate_fields_type(source_df):
  tb_schema = StructType(
      List(
        StructField("ResellerId",IntegerType, False),
...
rslr_df = validate_count('Reseller')

#check existence of columns
quinn.validate_presence_of_columns(rslr_df, ["ResellerId","FirstName","LastName", "CreateDate", "UpdateDate"])
#Check Columns Types
validate_fields_type(rslr_df)

#valid df
#df = rslr_df.withColumn('BatchId',BatchId)
df = rslr_df
#insert df to hive staging db
df.write.mode("append").saveAsTable("staging.evnt_pltfrm_reseller")
