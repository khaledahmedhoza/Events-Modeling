...
def validate_fields_type(source_df):
  tb_schema = StructType(
      List(
        StructField("EvResId",IntegerType, False),
        StructField("EvntId",IntegerType, False),
        StructField("ResId",IntegerType, False),
        StructField("Commission",DoubleType, False),
...
comm_df = validate_count('Commission')

#check existence of columns
quinn.validate_presence_of_columns(comm_df, ["EvResId", "EvntId", "ResId", "Commission", "CreateDate", "UpdateDate"])
#Check Columns Types
validate_fields_type(comm_df)

#valid df
#df = comm_df.withColumn('BatchId',BatchId)
df = comm_df
#insert df to hive staging db
df.write.mode("append").saveAsTable("staging.evnt_pltfrm_commission")
