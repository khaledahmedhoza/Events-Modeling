...
def validate_fields_type(source_df):
  tb_schema = StructType(
      List(
        ...
        StructField("TicketCost",DoubleType, False),
...
evnt_df = validate_count('Events')

#check existence of columns
quinn.validate_presence_of_columns(evnt_df, ["EventId","StartDate","EndDate", "VenueId","TicketCost", "EventType", "EventName", "LocId", "CreateDate", "UpdateDate"])
#Check Columns Types
validate_fields_type(evnt_df)

#valid df
#df = evnt_df.withColumn('BatchId',BatchId)
df = evnt_df
#insert df to hive staging db
df.write.mode("append").saveAsTable("staging.evnt_pltfrm_events")
